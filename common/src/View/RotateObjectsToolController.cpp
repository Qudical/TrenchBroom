    ...
    namespace View {
        namespace {
            ...
            public:
                ...
                DragConfig initialize(const InputState& inputState, const vm::vec3& initialHandlePosition) override {
                    ...
                    return DragConfig{
                        makeGetHandlePosition(
                            makeCircleHitFinder(center, axis, radius),
                            makeCircleSnapper(m_tool.grid(), m_tool.angle(), center, axis, radius)),
                        initialHandlePosition,
                        initialHandlePosition
                    };
                }

                std::optional<DragConfig> modifierKeyChange(const InputState&, const vm::vec3&, const vm::vec3&) override { return std::nullopt; }

                DragStatus drag(const InputState&, const vm::vec3& initialHandlePosition, const vm::vec3& /* lastHandlePosition */, const vm::vec3& currentHandlePosition) override {
                    const auto center = m_tool.rotationCenter();
                    const auto axis = m_tool.rotationAxis(m_area);
                    const vm::vec3 ref = vm::normalize(initialHandlePosition - center);
                    const vm::vec3 vec = vm::normalize(currentHandlePosition - center);
                ...
                void end(const InputState&, const vm::vec3& /* initialHandlePosition */, const vm::vec3& /* currentHandlePosition */) override {
                ...
                void render(const InputState& inputState, Renderer::RenderContext& renderContext, Renderer::RenderBatch& renderBatch, const vm::vec3& initialHandlePosition, const vm::vec3& /* currentHandlePosition */) const override {
                    m_renderHighlight(inputState, renderContext, renderBatch, m_area);
                    renderAngleIndicator(renderContext, renderBatch, initialHandlePosition);
            ...
            class RotateObjectsBase : public ToolControllerBase<NoPickingPolicy, NoKeyPolicy, MousePolicy, NoMouseDragPolicy, RenderPolicy, NoDropPolicy> {
            protected:
                RotateObjectsTool* m_tool;
            protected:
            ...
            private:
                ...
                std::unique_ptr<DragTracker> acceptMouseDrag(const InputState& inputState) override {
                    ...
                    return createHandleDragTracker(RotateObjectsDragDelegate{*m_tool, area, std::move(renderHighlight)}, inputState, initialHandlePosition);
                }
            ...
            public:
                ...
                DragStatus move(const InputState&, const vm::vec3& /* lastHandlePosition */, const vm::vec3& currentHandlePosition) override {
            ...
            protected:
                ...
                std::unique_ptr<DragTracker> acceptMouseDrag(const InputState& inputState) override {
                    ...
                    return createMoveHandleDragTracker(MoveRotationCenterDragDelegate{*m_tool, std::move(renderHighlight)}, inputState, m_tool->rotationCenter());
                }
